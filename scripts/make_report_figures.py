...
def fig_finetune_loss():
    d = load(os.path.join(REPORT_DIR, "..", "outputs", "results", "finetune_insight_report.json"))
    hist = d["history"]
...
# ---------------------------------------------------------------------------
def fig_linprobe():
    d = load(os.path.join(REPORT_DIR, "..", "outputs", "results", "comparison_report_v2.json"))
...
# ---------------------------------------------------------------------------
def fig_timit():
    d = load(os.path.join(REPORT_DIR, "..", "outputs", "benchmark", "deepfaketimit_report.json"))
    vit = d["vit"]
    cnn = d["cnn"]
