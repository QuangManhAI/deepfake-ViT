...
Nguồn:
  real known (cdf): data/real-root/Celeb-DF-v2/...          (đã trích theo JSON)
...
Chạy:
  .venv/bin/python scripts/build_test_data.py --dry-run
  .venv/bin/python scripts/build_test_data.py --n 1000
"""
import argparse
import csv
import json
...
def celeb_real_path(json_path):
    """deepfakes_detection_datasets/Celeb-DF-v2/... -> data/real-root/..."""
...
def plan_known(method, n, rng):
    ...
    for label, pool in (("real", real), ("fake", fake)):
        ...
        for vid, p in keep:
            if label == "real":
                src = celeb_real_path(p)      # data/real-root/Celeb-DF-v2/...
            else:
