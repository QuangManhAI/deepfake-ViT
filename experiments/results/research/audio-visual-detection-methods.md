...
> Đây là danh mục **phương pháp (model)**, không phải dataset. Dataset AV đã có ở `experiments/results/research/deepfake-datasets-catalog.md` §D và `experiments/results/research/audio-visual-datasets.md`.
...
- Nghiên cứu có hệ thống biểu diễn tự-supervised (vision + speech) cho bài AV detection — gần với thiết kế "DINOv3 + audio branch" mà chúng ta đang định làm (xem `experiments/results/research/research-gaps.md` Gap G).
...
- **Baseline AV đề xuất cho Gap G** (xem `experiments/results/research/research-gaps.md`): giữ cặp matched **DINOv3 ViT-S+/16 vs ConvNeXt-Tiny** làm visual branch + branch audio (Wav2Vec2 2.0 / AST) + fusion; so sánh visual-only / audio-only / fused trên **FakeAVCeleb 4-way** và DFDC subset — đúng protocol cross-dataset kiểu AVFF.
